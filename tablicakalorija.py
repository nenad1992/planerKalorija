...
def run_tablicakalorija():
    ...
    def prikazVrednosti():
        ...
        tree = ttk.Treeview(tablicakalorija, columns=("Id", "Namirnica", "Kalorije",
    ...
    def adding_values_in_database():
        conn = sqlite3.connect("tablicakalorija.db")
        cursor = conn.cursor()
        query = '''INSERT INTO tablicakalorija (namirnica, kalorije, proteini, ugljenihidrati, masti)
                    VALUES (?, ?, ?, ?, ?)'''
        cursor.execute(query,
                    (namirnica_entry.get(),
                    float(kalorije_entry.get()),
    ...
    def change_values_in_database():
        ...
        def change_values_in_database_ok():
            conn = sqlite3.connect("tablicakalorija.db")
            cursor = conn.cursor()
            query = '''UPDATE tablicakalorija SET namirnica = ?, kalorije = ?, proteini = ?, ugljenihidrati = ?, masti = ? WHERE id = ?'''
            cursor.execute(query,
                        (namirnica_entry.get(),
                        float(kalorije_entry.get()),
                        float(proteini_entry.get()),
            ...
            cursor.close()
            conn.close()
            messagebox.showinfo("Information", "Vrednost uspesno izmenjena!")
            namirnica_entry.delete(0, 'end')
            proteini_entry.delete(0, 'end')
            ugljenihidrati_entry.delete(0, 'end')
            masti_entry.delete(0, 'end')
        ...
        def on_row_selected(event):
            ...
            kalorije_entry.delete(0, 'end')
            kalorije_entry.insert(0, item_values[2])
            proteini_entry.delete(0, 'end')
            proteini_entry.insert(0, item_values[3])
            ugljenihidrati_entry.delete(0, 'end')
            ugljenihidrati_entry.insert(0, item_values[4])
            masti_entry.delete(0, 'end')
            masti_entry.insert(0, item_values[5])
